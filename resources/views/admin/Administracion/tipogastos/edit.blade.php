<x-admin-layout :breadcrumbs="[
    ...
    ['name' => 'Editar Tipo de Gasto']
]">
    <div class="p-6 bg-white w-full">
        <h2 class="text-xl font-bold mb-4 text-black">Editar Tipo de Gasto</h2>

        <form method="POST" action="{{ route('admin.administrativo.tiposgastos.update', $tipoGasto->id) }}" class="grid gap-4 w-full">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $tipoGasto->nombre) }}" required
            ...
            <div>
                ...
                <select name="categoria_gasto_id" id="categoria_gasto_id" required
                    ...
                    @foreach($categorias as $categoria)
                        <option value="{{ $categoria->id }}" {{ $tipoGasto->categoria_gasto_id == $categoria->id ? 'selected' : '' }}>
                            {{ $categoria->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion"
                          class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('descripcion', $tipoGasto->descripcion) }}</textarea>
            </div>
