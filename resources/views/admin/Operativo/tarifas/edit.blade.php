<x-admin-layout :breadcrumbs="[
    ...
    <div class="p-6 bg-white rounded-lg shadow">
        <h2 class="text-xl font-bold mb-4 text-black">Editar Tarifa</h2>

       <form method="POST" action="{{ route('admin.operativo.tarifas.update', $tarifa) }}" class="grid gap-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="ruta_id" id="ruta_id" class="block w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    @foreach($rutas as $ruta)
                        <option value="{{ $ruta->id }}" {{ $tarifa->ruta_id == $ruta->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="tipo_transporte_id" id="tipo_transporte_id" class="block w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    @foreach($tiposTransporte as $tipo)
                        <option value="{{ $tipo->id }}" {{ $tarifa->tipo_transporte_id == $tipo->id ? 'selected' : '' }}>
                            {{ $tipo->nombre_transporte }}
                        </option>
                    @endforeach
                </select>
            </div>
